...
1. Create a Node Workflow service from the `main` branch in this repository in Render: build `npm ci && pip install -r requirements.txt`, start `node workflows/tasks.js`. The root `.python-version` pins Python to 3.12.11 so the pinned scikit-learn artifact can load. The Python scorer and its dependencies are packaged in the workflow task image.
...
3. Set the Workflow service build command to `npm ci && pip install -r requirements.txt` (the repository `.python-version` pins the compatible Python runtime). Both patient check-in scoring and clinician analysis call `<slug>/monitoringPipeline`. The Workflow runs the Python scorer alongside the deterministic evidence chain and returns both plus a comparison and its Render run ID. It runs at check-in start, on patient submission, and when a clinician explicitly analyzes or simulates; live ElevenLabs turns stay direct for responsiveness.
...
| --------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| The rules and views                     | `npm test` , 67 JS unit tests, plus `node tests/api.integration.js` against a live server                                                                                                                                                                                                                                     |
| The safeguards, in the shipped mode     | `node tests/accountsMode.integration.js` runs the configuration the deployment uses — accounts required, data encrypted at rest — over invitation, session, role scoping, break-glass, the chained audit naming the actor, and server-side revocation                                                                         |
| The Python model                        | `PYTHONPATH=ml python -m unittest discover -s ml/tests` , 63 tests across contracts, features, scoring, scenarios                                                                                                                                                                                                             |
