...
This part of the repo holds the real-wearable ingestion pipeline and the validation work
...
Produces `fixtures/daily_deid.csv` (de-identified daily aggregates) and
`fixtures/evidence.json` (statistical evidence objects). Takes ~90 seconds on a
...
[INTEROP.md](INTEROP.md) for the numbers and the one-line fix.
...
Runs `shared/engine.js` against the real fixtures and prints the coverage
math, which metrics are gated off, and what the engine does on the days a
personal-baseline detector finds events. Exits non-zero while the engine
cannot flag real data, so it doubles as a regression check once fixed.
