...
export default function Root() {
  ...
  useEffect(() => {
    ...
    async function checkAccess() {
      try {
        ...
        if (live) setGate({ checked: true, required: true, role: null, error: true });
      }
    }
    checkAccess();
    return () => {
  ...
  if (!gate.checked) return <div className="rx rx-loading">Checking workspace access…</div>;
  if (gate.error) return (
    <div className="rx rx-loading">
      <p>Could not verify workspace access.</p>
      <button className="rx-btn" onClick={() => location.reload()}>Try again</button>
    </div>
  );
  const AccessScreen = section === "patient" ? SignIn : Login;
  if (gate.required && !gate.role)
    return (
      <div className="rx"><AccessScreen
  ...
  if (section === "welcome" || section === "about")
    return <div className="rx"><Landing /></div>;
  if (section === "login" && !gate.required)
    return <div className="rx"><Login /></div>;
